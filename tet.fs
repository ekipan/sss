\ \ tetris for durexforth 4. 'piece' is
\ the heart of this program, study it.
marker --tet-- decimal
...
6 . \ low level: c64 stuff, optim.

: else:  postpone exit           \ lang
...
\ w! save table addr for p@     \ optim
\ scan+add center (p)ositn. see piece.
...
2 field posn  \ $yyxx from bottom left.
1 field turns \ 0-3 clockwise turns.
1 field shape \ 0-6 shape idx ijltszo.
1 field held  \ 0-6 with pin bit 8.
8 field queue \ 0-6 next random shapes.
2 field qh    \ mod8 queue head index.
...
2 field oldp  \ drawn piece to erase.
2 field oldt
well - constant size
...
: row ( -a) posn 1+ c@ 10* well + ;

: drawn ( -pts) oldp @ oldt @ split ;
: curr ( -pts) posn @ turns @ split ;
: t@+ ( t-t) turns c@ + 3 and ;
: curr+ ( pt-pts) swap posn @ +
  swap t@+  shape c@ ;
: curr+! ( pt-) t@+ turns c!  posn +! ;
: enter ( -) $1305 posn ! 0 turns c! ;
: touch ( -) posn oldp 4 move ;
...
: r  kbinit bg #all d! curr piece hit?
